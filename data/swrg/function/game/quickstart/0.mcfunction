...
execute if entity @s[tag=!admin] unless entity @a[tag=admin] if score #count swrg.math matches ..1 run tellraw @s {"nbt":"title.no_players","storage":"swrg:lang","interpret":true}
...
execute if entity @s[tag=admin] if score #count swrg.math matches ..1 run tellraw @s {"nbt":"title.no_players","storage":"swrg:lang","interpret":true}
