execute if score ranked lbc.event matches 1 run return run function swrg:gui/select/3000_ranked
# НЕ ЗАБЫВАТЬ МЕНЯТЬ                                                  ^^^^^^^^^^^^^^^^^^^^


scoreboard players set @s[tag=swrg.kit1,scores={swrg.gui_select=0}] swrg.kit 1
scoreboard players set @s[tag=swrg.kit2,scores={swrg.gui_select=1}] swrg.kit 2
scoreboard players set @s[tag=swrg.kit3,scores={swrg.gui_select=2}] swrg.kit 3
scoreboard players set @s[tag=swrg.kit4,scores={swrg.gui_select=3}] swrg.kit 4
scoreboard players set @s[tag=swrg.kit5,scores={swrg.gui_select=4}] swrg.kit 5
scoreboard players set @s[tag=swrg.kit6,scores={swrg.gui_select=5}] swrg.kit 6
scoreboard players set @s[tag=swrg.kit7,scores={swrg.gui_select=6}] swrg.kit 7
scoreboard players set @s[tag=swrg.kit8,scores={swrg.gui_select=7}] swrg.kit 8
scoreboard players set @s[tag=swrg.kit9,scores={swrg.gui_select=8}] swrg.kit 9
scoreboard players set @s[tag=swrg.kit10,scores={swrg.gui_select=9}] swrg.kit 10
scoreboard players set @s[tag=swrg.kit11,scores={swrg.gui_select=10}] swrg.kit 11
scoreboard players set @s[tag=swrg.kit12,scores={swrg.gui_select=11}] swrg.kit 12
scoreboard players set @s[tag=swrg.kit13,scores={swrg.gui_select=12}] swrg.kit 13
scoreboard players set @s[tag=swrg.kit14,scores={swrg.gui_select=13}] swrg.kit 14
scoreboard players set @s[tag=swrg.kit15,scores={swrg.gui_select=14}] swrg.kit 15
scoreboard players set @s[tag=swrg.kit16,scores={swrg.gui_select=15}] swrg.kit 16
scoreboard players set @s[tag=swrg.kit17,scores={swrg.gui_select=16}] swrg.kit 17
scoreboard players set @s[tag=swrg.kit18,scores={swrg.gui_select=17}] swrg.kit 18
scoreboard players set @s[tag=swrg.kit19,scores={swrg.gui_select=19}] swrg.kit 20
scoreboard players set @s[tag=swrg.kit20,scores={swrg.gui_select=20}] swrg.kit 21
scoreboard players set @s[advancements={lbc:true_advancements/event/eat_halloween_food=true},scores={swrg.gui_select=21}] swrg.kit 22
scoreboard players set @s[advancements={lbc:true_advancements/event/halloween_mace=true},scores={swrg.gui_select=22}] swrg.kit 23
scoreboard players set @s[advancements={lbc:true_advancements/event/wind_wand_upgraded=true},scores={swrg.gui_select=23}] swrg.kit 24


tellraw @s[advancements={lbc:true_advancements/event/eat_halloween_food=false},scores={swrg.gui_select=21}] {"translate":"lbc.adv.event.eat_halloween_food.lore"}
tellraw @s[advancements={lbc:true_advancements/event/halloween_mace=false},scores={swrg.gui_select=22}] {"translate":"lbc.adv.event.halloween_mace.lore"}
tellraw @s[advancements={lbc:true_advancements/event/wind_wand_upgraded=false},scores={swrg.gui_select=23}] {"translate":"lbc.adv.event.upgraded_wind_wand.lore"}


#
scoreboard players set @s[scores={swrg.gui_select=18}] swrg.gui_page 0
scoreboard players set @s[scores={swrg.gui_select=26}] swrg.kit 0

scoreboard players set @s swrg.gui_select -1